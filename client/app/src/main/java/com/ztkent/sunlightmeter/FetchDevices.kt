...
/*
TODO: Will likely need this style?
class MyRepository {
    private var cachedData: MyDataType? = null

    suspend fun fetchData(): MyDataType {
        if (cachedData == null) {
            cachedData = fetchDataFromNetwork()
        }
        return cachedData!!
    }
}
...
private val semaphore = Semaphore(10) // Limit concurrency to 10

fun FetchAvailableDevices(context: Context): Deferred<List<String>> {
...
private fun checkForDeviceResponse(ip: String, host: String): Boolean {    var url = "http://$ip/id"
    if (host.isNotEmpty()) {
        url = "http://$host/id"
    }
    try {
